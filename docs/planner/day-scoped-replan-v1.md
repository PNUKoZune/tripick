...
- **일차 간 동선 연속성은 여전히 아무도 안 본다** — 2일차만 다시 짜면 1일차 마지막 장소와 2일차 첫 장소의 거리가 고려되지 않는다. 원래도 그랬지만(§6), 부분 재계획은 "나머지는 그대로 두겠다"는 약속이라 이 간극이 더 눈에 띌 수 있다
...
- **`/alternative/request` 의 트리거 유실** — 이 브랜치와 무관한 별도 버그. 컨트롤러가 바디의 trigger 를 `manual` 로 덮어써 배너에서 연 재계획이 서버에서 일반 재계획으로 처리된다. `fix/replan-trigger-passthrough` 브랜치에서 수정 중이며, 그게 머지돼야 범위와 트리거가 같이 살아난다
