...
선행 문서: [`docs/weather-alert-scheduler-v1.md`](./weather-alert-scheduler-v1.md)·[`docs/crowd-alert-scheduler-v1.md`](./crowd-alert-scheduler-v1.md) (동형 스케줄러·미러링 원본), [`docs/trip-progress-live-v1.md`](./trip-progress-live-v1.md) (라이브 위치·이탈 감지 원본), [`docs/inbox-and-trip-invite-v1.md`](./inbox-and-trip-invite-v1.md) (인박스·푸시), [`docs/mobile-webview-setup.md`](./mobile-webview-setup.md) (RN 브리지·위치 추적)
...
- **"수락 → 재계획" 미배선** — `arrival_alert`·`crowd_alert`·`weather_alert` 모두 알림 탭 시 planner 이동까지만이고, 거기서 재계획은 `manual` 로 나간다. `trigger:'deviation'` 은 타입·프롬프트가 준비됐으나 알림에서의 진입점이 없다. 함께 배선하는 것이 후속 후보
