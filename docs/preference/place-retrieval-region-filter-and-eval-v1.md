...
- **대표 명소 커버리지** (Tier 1) — 남산서울타워·설악산·성산일출봉이 전국 카탈로그에 없다. KTO `areaBasedList2` 는 인기순 정렬이 없어 깊이로는 못 잡는다. 지역별 대표 명소 키워드 시드를 카카오 키워드 검색으로 적재하는 별도 패스가 필요하다.
- **광주 커버리지 3건** — KTO 시도 목록의 `전남광주통합특별시` 통합 때문에 카카오 앵커가 전남으로 쏠린다. 앵커를 시군구 단위로 분산하거나, 코드 표에 없는 통합 라벨을 감지해 앵커를 나눠야 한다.
- **시군구 목적지의 경계 명소 누락** — '속초' 로 좁히면 인접 시군구에 걸친 설악산 권역이 후보에서 빠진다. 시군구 필터에 같은 시도 인접 시군구를 얹거나 좌표 반경으로 보완하는 안.
