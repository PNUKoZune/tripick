...
`feat/otp-routing` 이 GTFS+OSM 을 단일 그래프로 빌드해 자동차·대중교통·도보를 통합 조회하도록 만들어 뒀으나 폐기했다. 이유는 메모리 요건이다 — 전국 그래프는 **서빙에만 힙 13g** 이 필요하고, 로컬 16GB 에서는 LLM 과 동시 구동이 불가능하다. 자세한 실측은 폐기된 브랜치의 `docs/otp-routing-v1.md` 에 남아 있다.

외부 API 로 돌아오면서 잃은 것과 얻은 것:

| | OTP2 | 외부 API (현재) |
| --- | --- | --- |
| 로컬 구동 | LLM 과 동시 불가 (13g) | 제약 없음 |
| 도보 경로 | 실경로 | 직선거리 추정 (5절) |
| 출발 시각 반영 | `departAt` 지원 | 미지원 (9절) |
