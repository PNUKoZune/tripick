...
- [ ] 세종 `sigungu_code` 44건 오류 — 옛 백필 SQL 이 `'세종특별자치시 장군면 …'` 에서 `'장군면'` 을 시군구로 잡아 `'장'` 을 박았다(`'국책연구원3로'`→`'국책연'` 도). TS 파생은 null 이라 develop 에서도 불일치. 세종은 목적지가 시도로 잡혀 시군구 필터를 안 쓰므로 검색 영향 없음 — 고치려면 `sigungu_code` 전체 재파생 필요 ([merged-sido-label §6](../preference/merged-sido-label-v1.md))
- [ ] popular 패스가 짧은 일반어 상호를 통과시킴 — 광주 적재 40건 중 `다른`·`이유`·`이젠`·`조금더` 5건. 카카오에 실존 등록 상호이고 코퍼스에도 그 단어가 있어 두 관문을 정당하게 통과한다. 길이 하한만 두면 실제 2~3글자 명소(우도·오도)를 잃으니 코퍼스 상대빈도나 카테고리 상세를 함께 보는 규칙 필요
...
- [ ] 인지도를 후보 선발 단계에 얹기 `[보류: 지금은 이득 없음, 규모 커지면 카드]` — **프로토타입으로 측정하고 되돌렸다.** 런타임 코퍼스에 적재용 forward extraction 을 돌려 이름을 뽑고(인지도 인덱스는 언급 수만 세고 이름 목록이 없다) `name ILIKE ANY` 로 카탈로그를 직접 조회, 벡터 후보와 합집합. 메커니즘은 동작한다 — 벡터 상위 160 밖 후보를 케이스당 2~21건 끌어온다. **핵심 발견: 풀 배수의 대체재다** — 풀×3 에 선발만 얹으면 풀×10 과 동등(R\|cat 0.502 vs 0.498, recall@10 0.326 vs 0.317)한데 160개를 훑는 대신 이름 30개만 조회한다. 다만 **이미 ×10 을 쓰는 지금은 순수 손해**(R\|cat·recall 불변, MRR 0.720→0.674). 하락 원인은 카탈로그에 남아 있던 SEO 상호('경주맛집' 인지도 1.00)를 선발이 증폭한 것 — 아이디어의 결함이 아니라 데이터 품질 문제. 카탈로그가 10만 행대가 되거나 HNSW 경로로 밀려 정확 KNN 을 못 쓸 때 꺼내 쓸 카드로 남긴다(그때는 인지도를 행에 저장하는 설계가 런타임 추출보다 나을 수 있다) ([ranking-tuning §6](../preference/retrieval-ranking-tuning-v1.md))
- [ ] 카탈로그의 SEO 상호·여행코스 글 행 — `…맛집` 으로 끝나는 실질 SEO 상호 5건('경주맛집'·'다솥맛집' 류)은 인지도 1.00 을 받아 상위를 먹는다. 적재 단계 `isGenericPlaceName` 이 생기기 전 행들. **검색 단계에 그 규칙을 그대로 쓰면 안 된다** — `…코스`·`…여행` 접미 80건 중 '해파랑길 2코스'·'금정산둘레길 6코스'·'경기둘레길 가평20코스' 같은 실제 코스명이 대부분이라 오탈락한다. 별도로 KTO 여행코스 **글**('가정의 달, 싱글을 위한 혼자 먹는 밥상 코스')이 장소로 적재된 것도 섞여 있어, 접미사보다 정밀한 판별(주소 유무·카테고리)이 필요하다
- [ ] `retrieval` 항이 사실상 상수 — pgvector 유사도가 전 후보 0.73~0.82(코사인 0.46~0.64)에 몰려 가중 0.24 를 받으면서 순위를 못 가른다. 정규화·곡선 재조정으로 변별력을 만들 수 있으나 절대 confidence 의미와 accept 게이트(`CRAG_MIN_CONFIDENCE`)에 연동돼 있어 별 작업
- [ ] 검색 결과 근접 중복 제거 — 제주에서 '한라산' 행 둘 + '한라산국립공원'·'한라산 동능' 이 별 후보로 16칸을 나눠 쓴다. 적재 파이프라인엔 있는 이름+좌표 dedupe 가 검색 결과엔 없다
