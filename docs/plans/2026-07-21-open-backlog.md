...
- [ ] 광주가 전남으로 묻힘 — **진단 정정**: KTO 라벨만의 문제가 아니라 **카카오 주소 정본도 `전남광주통합특별시`** 다(행정구역 통합이 주소 체계에 반영됨). `toSidoCode` 가 '전남'으로 떨어뜨려 광주 소재 장소가 전부 전남으로 적재된다 — 로컬 카탈로그에 광주 자치구 주소인데 전남 분류인 행 212건, `region_code='광주'` 는 3건(옛 표기 잔존). '광주' 목적지 검색이 조용히 빈다. 해결 규칙은 명확: **통합 라벨에선 시군구 접미사가 시도를 가른다**(광주는 자치구만, 전남은 자치구 없음 → '구' 끝이면 광주, '시'·'군' 이면 전남). `region-code.ts` 코드 파생 의미 변경 + 212행 백필 마이그레이션이라 독립 작업 ([popular-place-ingestion §8.1](../preference/popular-place-ingestion-v1.md))
- [ ] 카탈로그에 있는데 상위에 못 오는 랭킹 문제(`R|cat` 0.168 / 커버리지 80%) — **이제 병목**. 커버리지가 47%→80% 로 오르며 분모가 커져 비율은 0.27→0.17 로 내려갔지만 절대 적중은 늘었다(recall@5·MRR 상승). "정답의 절반이 없는 상태에서 상수를 만지면 노이즈를 쫓는 셈"이라 미뤄 뒀던 blend weight·radius 스윕을 다시 돌릴 조건이 됐다
