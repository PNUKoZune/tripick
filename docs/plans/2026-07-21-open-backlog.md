...
- [ ] 인지도를 후보 선발 단계에 얹기 — 풀 배수로 급한 불은 껐지만 선발은 여전히 벡터 거리만 본다. 카탈로그 10만 행대에선 배수만으로 한계. 인지도 인덱스가 **이름 목록을 갖고 있지 않은**(코퍼스 문자열로 언급 수만 세는) 구조가 걸림돌 — forward extraction 을 런타임에 돌리거나 인지도를 행에 저장하는 설계 필요
