...
- [ ] 카탈로그에 있는데 상위에 못 오는 랭킹 문제(`R|cat` 0.173 / 커버리지 81%, 11케이스) — **이제 병목**. 커버리지가 47%→80% 로 오르며 분모가 커져 비율은 0.27→0.17 로 내려갔지만 절대 적중은 늘었다(recall@5·MRR 상승). "정답의 절반이 없는 상태에서 상수를 만지면 노이즈를 쫓는 셈"이라 미뤄 뒀던 blend weight·radius 스윕을 다시 돌릴 조건이 됐다
