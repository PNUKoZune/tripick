...
| 서빙 (`otp`) | `13g` | 그래프 로딩만으로 힙 ~10.5GiB. 10g 면 검색 여유가 없어 GC 폭주 |

> ⚠️ **물리 16GB(WSL2 ~15.4Gi) 로컬에서는 전국 그래프 + 13g 가 커널 OOM 킬을 맞는다.**
> 기본값은 메모리를 넉넉히 잡는 배포 머신 기준이다. 로컬에서 전국을 띄워야 하면
> `OTP_MAX_HEAP=10g` 로 낮출 수 있지만, 그러면 동시 대중교통 질의가 심하게 느려진다
> (같은 4구간 병렬이 1.35초 → 80초). 로컬은 **권역 그래프**를 권장한다.
> 자세한 실측은 [`docs/otp-routing-v1.md`](../../docs/otp-routing-v1.md) 6-3 절.
