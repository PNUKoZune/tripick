...
- 후보 검색 범위는 **행정구역 코드 등가 비교가 기본**이고, 그걸로 안 잡히는 목적지만 **좌표 앵커 + 반경**으로 돈다. `destinationRegionFilter` 는 임의 문자열에서도 시군구 코드를 만들어 내므로 '광안리'·'남이섬' 같은 입력은 실재하지 않는 코드가 되어 후보가 **0건**이 됐다(그 뒤 카카오 폴백 → 서울 좌표 DEFAULT_SEEDS). `DestinationAnchorService` 가 카카오 로컬 키워드 검색으로 좌표와 지번 주소를 얻고, 주소에서 `placeRegionCodes` 로 정본 지역 코드를 파생한다. **시도가 잡히거나 시군구 코드로 카탈로그 후보가 나오는 목적지는 이 경로를 안 탄다** — '해운대'(61행)·'경주'(382행)는 기존 동작 그대로다. 반경은 2→5→10km 로 넓히고(고정 반경은 광안리 2km 식당 2곳 / 에버랜드 10km 15건을 동시에 못 맞춘다) 그래도 얇으면 시도 전역을 **덧댄다**(교체 아님). ⚠️ 앵커 이름이 목적지를 포함하지 않으면 해석을 포기하고 기존 경로로 돌아간다 — '서면' 은 카카오가 순천시 서면(행정 면)을 주므로 그대로 쓰면 부산 여행이 전남 여행이 된다. 이 모호성의 정본 해결은 목적지 입력에서 사용자가 후보를 고르게 하는 것
- 앵커 목적지의 `locality` 항은 **지역 코드가 아니라 앵커까지의 거리**로 매긴다. 반경으로 후보를 좁혀 놓아도 그 안은 전부 같은 시도라 코드 비교가 전 후보 0.92 일률이 되어 아무것도 못 가른다(광안리 2위가 2km 밖 센텀시티 스파랜드였다). 거리는 **확정 반경으로 정규화**한다 — 고정 밴드(0.5/2/5/12km)를 쓰면 2km 앵커 안에서 두 밴드에만 걸려 서면역 지표가 꿈쩍도 안 했다. 세기(`CRAG_ANCHOR_EDGE_SCORE`) 무릎은 0.7 이고 0.6 부터는 거리가 과해져 정답을 잃는다(R|cat 0.540→0.495)
- 검색 품질 지표(`pnpm eval:retrieval`)의 **전후 비교는 한 프로세스 안에서만 유효하다**. 인지도 코퍼스가 네이버 실시간 검색이라 시간이 지나면 바뀌고(캐시는 프로세스 내부), 그 변동폭(R@10 ±0.013)이 웬만한 튜닝 효과보다 크다. 몇 분 내 반복은 소수점까지 동일하지만 30분 뒤는 다르다 — 실제로 태그 사전 확장이 시점 다른 비교에서 "R@10 +0.006, 제주 MRR 0.50→1.00" 으로 보였는데 연속 A/B 에서는 **완전히 동일**했다. 파라미터는 `--sweep`(한 프로세스), 코드 변경은 노브로 빼서 스윕하거나 몇 분 내 연속 실행으로 비교할 것
- 네이버 인지도는 **코퍼스가 그 지역을 담아냈을 때만** 켠다(`CRAG_MIN_POPULARITY_COVERAGE`, 후보 풀 언급 비율 5% 하한). 네이버 검색 `description` 이 본문이 아니라 **120자 스니펫**이라 "대구 실내 여행지 6곳" 글에서 정작 그 6곳 이름이 코퍼스에 안 들어간다 — 신호가 잡히는 비율이 대구 1.3% ~ 속초 59.2% 로 **46배** 벌어진다. 그 상태의 '언급 0' 은 마이너가 아니라 정보 없음이라, 대구 서문시장·팔공산·83타워가 전부 0.15 를 받아 두 글자 일반 상호(중립 0.50)에게 밀렸다(서문시장 157건 중 31위). ⚠️ **항을 끄는 게 목적이 아니다** — 항상 끄면 골든셋 R|cat 이 0.631→0.364 로 무너진다. `localityJudgeable` 과 같은 "가를 수 없으면 그 항만 끈다" 원칙
- pgvector 반경 검색은 `lat`/`lng` **생성 컬럼**(coordinates jsonb 파생) + `(lat,lng)` btree 로 돈다. bbox 로 인덱스를 타고 정확 거리로 모서리를 깎는 2단 조건인데 **둘 다 필요하다** — jsonb 표현식은 인덱스를 못 타 전체 스캔(26.8ms→0.8ms)이고, bbox 만 쓰면 대각선이 반경의 1.41배라 밀집 지역이 딸려 온다(에버랜드 10km 원 15건 vs bbox 54건, 늘어난 대부분이 분당)
- **적재 게이트와 검색 게이트는 같은 함수로, 같은 입력으로 판정한다**(`isEligibleItineraryCandidate`). 적재가 SEO 상호만 거르던 시절엔 검색이 절대 후보로 안 쓰는 행(약국·의원)이 적재만 되고 쌓였고, 정리 CLI 로 걷어내도 재적재가 그대로 되돌려 놨다(부산 재적재 후 13건 재유입). ⚠️ 적재에서 `categoryDetail` 을 넘기면 안 된다 — place_embeddings 가 그 값을 저장하지 않아 **검색 단계 pgvector 후보에는 categoryDetail 이 없다**. 적재만 그걸 보면 게이트가 검색보다 후해져서 '저장은 되는데 검색엔 절대 안 나오는 행'이 계속 생긴다. ⚠️ 이름 기반 의료 필터는 오탈락이 있다 — '부산 구 백제병원'(등록문화재)·'충북대학교병원 치유의나눔길'이 함께 죽는다. KTO 는 이들을 관광지(12)로, 약국을 쇼핑(38)으로 주므로 소스 카테고리를 저장하면 갈릴 수 있지만 '더바디성형외과의원'도 관광지(12)라 그것만으론 부족하다
- KTO 축제공연행사(contentTypeId=15)는 **장소가 아니라 기간이 있는 이벤트**다. `event_start_date`/`event_end_date` 를 함께 적재하고 **검색 시점에 여행 날짜와 겹칠 때만** 후보로 남긴다(NULL=상시). 적재 때만 거르면 오늘 적재한 8월 축제를 10월 여행에 내주게 된다. 기간은 목록 API(areaBasedList2)에 없어 `searchFestival2` 로 따로 조달한다(시도당 1~2콜). ⚠️ 기간은 임베딩 텍스트 밖이라 텍스트 해시가 같으면 증분 적재가 통째로 건너뛴다 — 영업시간과 같은 별도 backfill 경로(`updateEventPeriod`)가 필요하고, **연례 축제는 같은 contentId 의 날짜가 매년 바뀌므로** 이게 없으면 작년 날짜에 갇혀 영영 안 보인다. 일자별 재계획은 그 일차 하루를 구간으로 쓴다(`RetrievalContext.visitWindow`, 영업시간용 `startAt` 과 별개)
- KTO 쇼핑(contentTypeId=38)은 **일부만 적재한다**. 전통시장('경주 중앙시장'·'강경젓갈시장')은 여행지지만 체인 매장('다이소 부산서면점')과 건물 입점 점포('구찌 롯데백화점 부산본점')는 아니다. `isRetailBranchOutlet` 이 이름의 지점 접미(`… 지역점`)와 주소의 층 표기로 가른다. ⚠️ 이 규칙은 **쇼핑 버킷 안에서만** 돌려야 한다 — 카탈로그 전체에 적용하면 카카오 소스 카페·식당 지점('스타벅스 해운대점')이 함께 죽는데 그건 실제로 일정에 넣는 장소다. 지운 이유는 반경 검색이 붙으면서 이게 결과를 먹었기 때문이다(서면역 2km 후보 33건 중 9건이 롯데백화점 입점 브랜드). 이미 들어온 행은 `pnpm cleanup:catalog -- --only=retail` 로 걷는다
