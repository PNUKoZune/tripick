...
export class PlaceRetrievalService {
   ...
   *
   * 20 이 무릎이다 — R@10 이 최고이고 비용은 +7ms 뿐이다. 40 은 R\|cat 을 0.017 더 얻지만
   * R@10 이 내려가고, 80 은 비용이 2.3배인데 이득이 거의 없다.
   *
  ...
  private candidatePoolMultiplier(): number {
    const value = this.readNumber('PLACE_CANDIDATE_POOL_MULTIPLIER', 20);
