 ...
 * `motion-safe:` 로 감싸 `prefers-reduced-motion: reduce` 에서는 정지한 회색 블록으로 남는다.
 *
...
export function Skeleton({ className = '' }: { className?: string }) {
  return (
    <span
      aria-hidden
      className={`block motion-safe:animate-pulse rounded-full bg-[color:var(--line,#EEF1F5)] ${className}`}
    />
  );
}
