...
export function InfoTooltip({
  ...
  return (
    <span className={`group/info relative inline-flex ${className ?? ''}`}>
      ...
      <span
        role="tooltip"
        className={`pointer-events-none absolute bottom-[calc(100%+6px)] z-10 ${ALIGN_CLASS[align]} ${widthClass} rounded-[8px] bg-[#191F28] px-2.5 py-1.5 text-[11px] font-medium leading-[16px] text-white opacity-0 shadow-lg transition-opacity duration-150 group-hover/info:opacity-100 group-focus-within/info:opacity-100`}
      >
        {message}
      </span>
    </span>
