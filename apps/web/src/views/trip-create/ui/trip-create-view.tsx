...
function TripCreateContent({ initialDestination }: { initialDestination?: string }) {
  ...
  const formBody = (
    <div className="space-y-6">
      ...
      >
        <div className="mb-3 flex items-center justify-between rounded-[12px] bg-[#F7F8FA] px-3.5 py-2.5">
          <span className="text-[13px] font-semibold text-[#191F28]">모든 날 같은 지역</span>
          <button
            type="button"
            role="switch"
            ...
            className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${
              sameRegion ? 'bg-[#3182F6]' : 'bg-[#D1D6DB]'
            }`}
          >
            <span
        ...
        {sameRegion ? (
          ...
          <p className="rounded-[12px] border border-dashed border-[#E5E8EB] px-4 py-5 text-center text-[13px] text-[#8B95A1]">
...
function DayRegionAccordionItem({
  ...
  return (
    <div className="overflow-hidden rounded-[14px] border border-[#E5E8EB] bg-white">
      ...
      >
        <span className="shrink-0 text-[13px] font-bold text-[#191F28]">{dayNo}일차</span>
        {dateLabel ? <span className="shrink-0 text-[12px] text-[#8B95A1]">{dateLabel}</span> : null}
        <span className="ml-auto flex min-w-0 flex-1 items-center justify-end gap-1.5">
          {regions.length > 0 ? (
            <span className="truncate text-[13px] font-semibold text-[#3182F6]">
              {regions.join(' · ')}
            </span>
          ) : (
            <span className="shrink-0 text-[13px] text-[#B0B8C1]">지역 선택</span>
          )}
          <LuChevronDown
            className={`size-4 shrink-0 text-[#8B95A1] transition-transform ${expanded ? 'rotate-180' : ''}`}
          />
        </span>
      </button>

      {expanded ? (
        <div className="border-t border-[#F2F4F6] px-3.5 pb-3.5 pt-3">
          {regions.length > 0 ? (
            <div className="mb-2 flex flex-wrap gap-1.5">
              {regions.map((region, index) => (
                <span
                  key={`${region}-${index}`}
                  className="inline-flex h-8 items-center gap-1 rounded-full bg-[#EDF3FF] pl-3 pr-1 text-[13px] font-semibold text-[#3182F6]"
                >
                  {region}
                  <button
                    type="button"
                    ...
                    className="flex size-5 items-center justify-center rounded-full text-[#5A93F0] hover:bg-[#DCE8FF]"
                  >
          ...
          <div className="flex items-start gap-2">
            ...
            <button
              ...
              disabled={!draft.trim()}
              className="flex h-12 items-center gap-1 rounded-[14px] bg-[#3182F6] px-3.5 text-[14px] font-semibold text-white transition disabled:bg-[#D1D6DB]"
            >
