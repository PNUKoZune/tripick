    ...
    <AppFrame showNav={false}>
      <div className="mx-auto flex min-h-dvh w-full max-w-[440px] flex-col px-5 pb-10 pt-12">
        <header className="mb-8">
          ...
          <h1 className="mt-2 text-[26px] font-bold text-[#191F28]">새 비밀번호 설정</h1>
        </header>

        {token ? (
          <ResetPasswordForm token={token} />
        ) : (
          <div className="rounded-[16px] border border-[#FECDD3] bg-[#FFECEE] p-5">
            <h2 className="text-[15px] font-bold text-[#F04452]">잘못된 링크</h2>
            <p className="mt-2 text-[13px] leading-[20px] text-[#4E5968]">
              토큰이 없거나 만료된 링크에요. 비밀번호 재설정을 다시 요청해주세요.
            </p>
            <Link
              href="/forgot-password"
              className="mt-3 inline-flex h-10 items-center rounded-[10px] bg-[#F04452] px-4 text-[13px] font-bold text-white"
            >
              다시 요청하기
            </Link>
          </div>
