...
/**
 * 인기 여행지를 카드로 노출해 원탭으로 여행 생성(프리필)에 진입시킨다.
 * 현재 추천은 관광공사 인기 지역 기반이며, 취향 개인화는 추후 백엔드 과제.
 */
export function DestinationSuggestions() {
  const { data = [] } = useQuery({
    queryKey: queryKeys.planner.destinations(''),
    queryFn: () => fetchDestinationSuggestions(''),
  ...
  return (
    <section className="mt-8">
      <div className="mb-3 flex items-baseline justify-between px-4 lg:px-0">
        ...
        <span className="text-[12px] text-[#8B95A1]">인기 여행지</span>
      </div>
